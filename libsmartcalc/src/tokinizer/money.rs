...
fn money_test_1() {
    ...
    assert_eq!(tokens[0].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
    
    assert_eq!(tokens[1].start, 8);
    assert_eq!(tokens[1].end, 15);
    assert_eq!(tokens[1].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
    
    assert_eq!(tokens[2].start, 16);
    assert_eq!(tokens[2].end, 24);
    assert_eq!(tokens[2].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
    
    assert_eq!(tokens[3].start, 25);
    assert_eq!(tokens[3].end, 32);
    assert_eq!(tokens[3].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
    
    assert_eq!(tokens[4].start, 33);
    assert_eq!(tokens[4].end, 41);
    assert_eq!(tokens[4].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
    
    assert_eq!(tokens[5].start, 42);
    assert_eq!(tokens[5].end, 49);
    assert_eq!(tokens[5].token_type, Some(TokenType::Money(1000.0, conf.get_currency("try".to_string()).unwrap())));
}
...
fn money_test_2() {
    ...
    assert_eq!(tokens[0].token_type, Some(TokenType::Money(2000.0, conf.get_currency("usd".to_string()).unwrap())));
}
